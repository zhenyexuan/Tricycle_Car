...
IFX_INTERRUPT(cc60_pit_ch0_isr, 0, CCU6_0_CH0_ISR_PRIORITY)
{
    ...
    Get_Speed();    //编码器获取速度
    printf("%d,%d,%d,%d,%.3f,%.3f,%.3f \n",encoder_Left,encoder_Right,Target_Speed1,Target_Speed2,LSpeed_PID.kp,LSpeed_PID.ki,LSpeed_PID.kd);
//    printf("%d,%d,%d,%d \n",ADC_Left_temp,ADC_Left_Angle_temp,ADC_Right_temp,ADC_Right_Angle_temp);
    Motor_PID();
    if(car_run_flag==1){
        motor_run(MotorDutyL,MotorDutyR);   //电机驱动
    }
}
